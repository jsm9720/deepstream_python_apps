...
def tracker_src_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            ...
            scale_x = original_width / STREAMMUX_WIDTH
            scale_y = original_height / STREAMMUX_HEIGHT

            left_orig = obj_meta.rect_params.left * scale_x
            top_orig = obj_meta.rect_params.top * scale_y
            width_orig = obj_meta.rect_params.width * scale_x
            height_orig = obj_meta.rect_params.height * scale_y

            # 정규화
            norm_left = round(left_orig / STREAMMUX_WIDTH, 6)
            norm_top = round(top_orig / STREAMMUX_HEIGHT, 6)
            norm_width = round(width_orig / STREAMMUX_WIDTH, 6)
            norm_height = round(height_orig / STREAMMUX_HEIGHT, 6)

            # 디버그 출력
            print(f"Object ID: {object_id}, Class: {class_name}")
            print(f"Box (normalized) - left: {norm_left}, top: {norm_top}, width: {norm_width}, height: {norm_height}")

            # bbox info append in array
            obj_info.append(
                {
                    "object_id": object_id,
        ...
        if len(obj_info) > 0:
            detect_data = {
                "device_id": source_id,
                "timestamp": ts,
                "detected_events": obj_info
            }
            socket.send_json(detect_data)
...
def main(args):
    # ZeroMQ Publisher setting
    context = zmq.Context()
    socket = context.socket(zmq.PUB)
    socket.bind("tcp://*:8554")
    ...
    if not tracker_src_pad:
        ...
        tracker_src_pad.add_probe(Gst.PadProbeType.BUFFER, tracker_src_pad_buffer_probe, socket)
    ...
    # cleanup
        ...
        print("Cleaning up pipeline and ZeroMQ...")
